...
if __name__=="__main__":
    rov = api.rover()
    goal_loc = lib.location()
    goal_loc.setLoc(100, 100, 0)
    ...
    while True:
        ...
        for dist in obs_dists:
            obs_wt = -1 * numpy.sign(obs_dists.index(dist) - 1) * obs_gain / (dist*dist)
            tot_obs_wt += obs_wt

        print("Heading to goal ", lib.relative_heading(rov_state.pose.position, goal_loc), " Obstacle weight ", tot_obs_wt)
        ...
        goal_heading = lib.relative_heading(rov_state.pose.position, goal_loc) + tot_obs_wt
